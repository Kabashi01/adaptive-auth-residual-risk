...
;; Impact of off-site location (remote maintenance)
(assert
  (ite (> Location 0)
       (and (= ConfPriority 0.68)
            (= AuthentPriority 0.6)
            (= IntegPriority 0.6)
            (= EffectPriority 0.6)
            (= EfficPriority 0.55)
...
(assert (= CalibrateSensorsRebaseline 1))

(assert (= RunSystemSelfTests 1))
(assert (= ExportDownloadLogs 1))
(assert (= UploadFirmware 1))
(assert (= OverrideSafetyLimits 1))
