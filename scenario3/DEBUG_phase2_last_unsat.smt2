...
;; -------------------------
;; The third scenario contextual factors
...
(assert (ite (> UnsecuredWiFi 0)
  (and
    (= PReplayAttack 0.6)       ;; Replay risk high: attacker may intercept and reuse tokens
    (= PSessionAttack 0.61))     ;; Session hijacking very likely over open networks
  true))
...
(assert (ite (> UnknownDevice 0)
  (and
    (= PImpersAttack 0.6)      ;; Impersonation risk high: compromised or spoofed identity
...
(assert (ite (> Location 0)
  (and
    (>= PImpersAttack 0.58)       ;; Impersonation risk moderate-high: user location is suspicious
    (= PSessionAttack 0.61))     ;; Session hijacking risk elevated: access from unknown place
  true))
